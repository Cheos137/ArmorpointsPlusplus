...
public class HealthTextComponent implements IRenderComponent {
	...
	public boolean render(RenderContext ctx) {
		...
		int hpcol  = ctx.data.isFullyFrozen()
				? ctx.config.hex(HexOption.TEXT_COLOR_FROSTBITE)
				: ctx.data.isEffectActive(ctx.data.effects().poison())
						? ctx.config.hex(HexOption.TEXT_COLOR_POISON)
						: ctx.data.isEffectActive(ctx.data.effects().wither())
								? ctx.config.hex(HexOption.TEXT_COLOR_WITHER)
								: ctx.config.hex(HexOption.TEXT_COLOR_HEART);
		...
		if(health < maxHp) {
			text.append(new RenderableText("/"   ).padRight(1).withColor(ctx.config.hex(HexOption.TEXT_COLOR_SEPARATOR )));
			text.append(new RenderableText(maxHp ).padRight(1).withColor(hpcol));
		}
		
		if(absorb > 0) {
			text.append(new RenderableText("+"   ).padRight(1).withColor(ctx.config.hex(HexOption.TEXT_COLOR_SEPARATOR )));
			text.append(new RenderableText(absorb).padRight(1).withColor(ctx.config.hex(HexOption.TEXT_COLOR_ABSORPTION)));
		}
		
		if (freeze > 0 && ctx.config.bool(BooleanOption.FROSTBITE_TEXT_ENABLE)) {
			text.append(new RenderableText(","   ).padRight(1).withColor(ctx.config.hex(HexOption.TEXT_COLOR_SEPARATOR)));
			text.append(new RenderableText(freeze).padRight(1).withColor(ctx.config.hex(HexOption.TEXT_COLOR_FROSTBITE)));
			text.append(new RenderableText("%"   ).padRight(1).withColor(ctx.config.hex(HexOption.TEXT_COLOR_SEPARATOR)));
		}
