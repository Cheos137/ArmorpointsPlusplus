...
public interface IConfig {
	...
	public static enum BooleanOption implements Option<Boolean> {
		...
		TOUTHNESS_ENABLE         ("showToughness"          , true , Category.GENERAL      , " Show toughness as overlay over armor"          , " Available: true, false [default: %s]");
		
		final String key;
		final boolean def;
		final Category category;
		final List<String> comments;
		BooleanOption(String key, boolean def, Category category, String... comments) {
			this.key = key;
			this.def = def;
	...
	public static enum IntegerOption implements BoundedOption<Integer> {
		...
		final Category category;
		final List<String> comments;
		IntegerOption(String key, int def, Category category, String... comments) {
			this.key = key;
			this.def = def;
			this.category = category;
	...
	public static enum FloatOption implements BoundedOption<Float> {
		...
		TOUGHNESS_VALUE ("toughness"   , 2F   ,    10, Category.REPRESENTATIVE, " Number of icons to show a armor toughness of 1"        , " Available: %s ~ %s [default: %s]");
		
		final String key;
		final float def, min, max;
		final Category category;
		final List<String> comments;
		FloatOption(String key, float def, float max, Category category, String... comments) { this(key, def, 0, max, category, comments); }
		FloatOption(String key, float def, float min, float max, Category category, String... comments) {
			this.key = key;
			this.def = def;
			this.min = min;
			this.max = max;
	...
	public static class EnumOption<T extends Enum<T>> implements Option<T> {
		...
		public static final EnumOption<Suffix.Type>
				SUFFIX          = new EnumOption<>("suffix"                    , Suffix.Type.SI     , Category.GENERAL    , " Suffix type used for displaying armor values"         , " Available: %s [default: %s]");
		public static final EnumOption<Alignment>
				...
				HEALTH_TEXT_ALIGNMENT = new EnumOption<>("healthValueAlignment", Alignment.RIGHT    , Category.TEXT_CONFIG, " if enabled, custom alignment of the helath value text", " Available: %s [default: %s]");
