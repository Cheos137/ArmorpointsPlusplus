...
public class RendererImpl implements IRenderer {
	...
	public void blit(IPoseStack poseStack, int x, int y, float u, float v, int width, int height, int texWidth, int texHeight) {
		((PoseStackImpl) poseStack).getGraphics().blit(RenderType::guiTextured, this.tex, x, y, u, v, width, height, texWidth, texHeight, this.color);
	}
	...
	public void blitM(IPoseStack poseStack, int x, int y, float u, float v, int width, int height, int texWidth, int texHeight) {
		...
		((PoseStackImpl) poseStack).getGraphics().drawSpecial(bufferSource -> {
			VertexConsumer vertexConsumer = bufferSource.getBuffer(RenderType.guiTextured(this.tex));
			vertexConsumer.addVertex(mat, x,         y         , 0).setUv((u + width) / texWidth, (v         ) / texHeight).setColor(this.color);
			vertexConsumer.addVertex(mat, x,         y + height, 0).setUv((u + width) / texWidth, (v + height) / texHeight).setColor(this.color);
			vertexConsumer.addVertex(mat, x + width, y + height, 0).setUv((u        ) / texWidth, (v + height) / texHeight).setColor(this.color);
			vertexConsumer.addVertex(mat, x + width, y         , 0).setUv((u        ) / texWidth, (v         ) / texHeight).setColor(this.color);
		});
	}
